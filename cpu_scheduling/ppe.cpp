bool comparePpe(struct process p1,struct process p2){
	return p1.priority  < p2.priority;
}

void checkCurrPpe(int i){
		...
		if(cpu.size()>0){
			if(currCpu.pid == -1){
				...
				currCpu = cpu[0];
				cpu.erase(cpu.begin());
				sort(cpu.begin(),cpu.end(),comparePpe);
			}
		}
...
void insertInQueuePpe(struct process p1){
	...
	if(p1.oper.size()>0){
		if( p1.oper[0][0] == 1 ){
			...
			inp.push_back(p1);
			// sort(inp.begin(),inp.end(),comparePpe);

			
		}else if( p1.oper[0][0] == 3){
			out.push_back(p1);
			// sort(out.begin(),out.end(),comparePpe);
		}
...
void startExecutionPpe(){
	while(!( (processBuffer.size() == 0) &&( out.size() == 0) && (inp.size() == 0) &&( cpu.size() == 0 ) && currCpu.pid == -1 && currInp.pid == -1 && currOut.pid == -1 )){
		...
		for(i = 0;i<inpbuff.size();i++){
			insertInQueuePpe(inpbuff[i]);
		}
		inpbuff.clear();

		for(i =0 ;i<outbuff.size();i++){
			insertInQueuePpe(outbuff[i]);
		}
		outbuff.clear();

		for(i =0 ;i<cpubuff.size();i++){
			insertInQueuePpe(cpubuff[i]);
		}
		cpubuff.clear();
		...
		while(i < processBuffer.size() && currTime >= processBuffer[i].arrivalTime && processBuffer[i].oper.size() > 0 ){
			...
			insertInQueuePpe(processBuffer[i]);	
...
void PPE( vector < struct process > inputProcess){
